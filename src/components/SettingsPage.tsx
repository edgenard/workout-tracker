...
function ExercisePlanEditor({ day, section, plan, movements, onChange }: { day: DayId; section: keyof Workout; plan: ExerciseTrainingPlan<TrainingFormat>; movements: Array<Movement>; onChange: (next: ExerciseTrainingPlan<TrainingFormat>) => void }) {
  ...
  return <div className="border-b border-zinc-800 py-4 last:border-b-0">
    <div className="flex flex-wrap items-center gap-3">
      <MovementSelect value={exercise.id} movements={movements} onChange={(id) => { const movement = movements.find((candidate) => candidate.id === id); if (movement) onChange({ ...plan, exercise: movement }) }} />
